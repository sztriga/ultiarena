"""parti vs multi[parti+ulti] — DEFENDER-SIDE silent ulti denial ablation.

Roles swapped vs eval_multi_ablation.py. God plays sol in both games
(parti-only — no awareness of silent ulti); defenders vary.

  Engine A (baseline): defenders = vanilla parti PIMC
  Engine B (multi):    defenders = multi PIMC with parti_pts=1.0, silent_ulti=2.0

Hypothesis: multi-aware defenders push god's parti-optimal line into
states where the trump-7 doesn't land in the last trick — denying
god's accidental silent ulti pickups. Since ulti-biased deals always
put trump-7 in sol's hand, defender silent-ulti columns are
structurally 0; we measure how often *sol* takes silent ulti and
whether multi-aware defense suppresses it.
"""
...
def _play_one(*, deal, def_contract, pimc_n, seed):
    """Play a full deal. Sol uses god (parti — no silent ulti awareness);
    defenders use PIMC with the given contract."""
    ...
    while not pis_bridge.is_terminal(pos):
        ...
        if p == 0:
            ...
            chosen = _pimc_pick(
                pos=pos, contract=def_contract, n_samples=pimc_n,
...
def worker(args):
    ...
    final_parti = _play_one(deal=deal, def_contract='parti',
                            pimc_n=PIMC_N, seed=seed)
    final_multi = _play_one(deal=deal, def_contract='multi',
                            pimc_n=PIMC_N, seed=seed)
...
def main():
    ...
    for label, idx in [("defenders=parti (baseline)", 1),
                       ("defenders=multi[parti+ulti]", 2)]:
