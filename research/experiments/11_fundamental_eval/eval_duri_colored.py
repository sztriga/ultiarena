"""Fundamental-contract eval — durchmars (colored).

Same template as eval_ulti.py. Per (α, seed):
  1. Build the deal (deal_durchmars_colored: 10-card sol + 2-card talon,
     fat trump suit, no mandatory ranks).
...
Duri has no auction must-hold rule (unlike ulti's trump-7), so the
determinizer's `_no_must_hold` is correct here.
...
from eval.dojo import deal_durchmars_colored
...
ALPHAS    = [0.60, 1.20, 2.00]
N         = 200
PIMC_N    = 32
N_WORKERS = 8
SEED_BASE = 82_000_000
OUT_DIR   = Path(__file__).parent
CHECKPOINT_PATH = OUT_DIR / "checkpoint_duri_colored.jsonl"
...
def main() -> None:
    ...
    (OUT_DIR / "results_duri_colored.json").write_text(json.dumps(out, indent=2))
    print(f"\nwrote {OUT_DIR / 'results_duri_colored.json'}")
