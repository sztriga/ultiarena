"""Fundamental-contract eval — durchmars (colorless).

Same template as eval_ulti.py. Per (α, seed):
  1. Build the deal (deal_durchmars_colorless: trumpless, 10-card sol
     biased toward dominating ranks). 2-card talon is dealt randomly.
  2. God-label the opening — does god predict sol can take all 10 tricks?
...
Colorless duri uses betli-style rank ordering (10 demoted under J), wired
via `solvers/pis.build_position` setting `is_colorless_duri=True` and
`training_mode="durchmars"`. No auction must-hold rule.
...
ALPHAS    = [0.80, 1.50, 2.50]
N         = 200
PIMC_N    = 32
N_WORKERS = 8
SEED_BASE = 83_000_000
OUT_DIR   = Path(__file__).parent
CHECKPOINT_PATH = OUT_DIR / "checkpoint_duri_colorless.jsonl"
...
def worker(args):
    alpha, seed = args
    deal = deal_durchmars_colorless(seed=seed, alpha=alpha)
...
def main() -> None:
    ...
    (OUT_DIR / "results_duri_colorless.json").write_text(json.dumps(out, indent=2))
    print(f"\nwrote {OUT_DIR / 'results_duri_colorless.json'}")
