"""Fundamental-contract eval — betli only.

Same template as the (now-removed) cross-contract eval. Per (α, seed):
  1. Build the deal.
  2. God-label the opening position (binary: does god predict a sol win?).
  3. Compute PIMC32 value at t=0 from the soloist's seat — the value head.
  4. Play two head-to-head games:
       a) PIMC sol  vs  god def  → did soloist win?  → sol_hold
       b) god  sol  vs  PIMC def → did soloist win?  → def_stop
     PIMC seat uses ``eval.pimc_matchup.pimc_pick`` (defender-aware).
...
Parallel via multiprocessing.Pool (8 workers default). Per-result
checkpoint to ``checkpoint.jsonl`` so a mid-run kill loses nothing.
"""
...
_CONTRACT     = "betli"
_VALUE_SCALE  = 10.0       # WIN=10, LOSE≈0 → divide by 10 to get [0, 1]

ALPHAS    = [0.30, 0.70, 1.50]
N         = 200
PIMC_N    = 32
N_WORKERS = 8
SEED_BASE = 80_000_000
OUT_DIR   = Path(__file__).parent
CHECKPOINT_PATH = OUT_DIR / "checkpoint_betli.jsonl"
...
def main() -> None:
    ...
    # rows shape: (alpha, label, pred, pimcsol_won, godsol_won) — drop seed
    ...
    (OUT_DIR / "results_betli.json").write_text(json.dumps(out, indent=2))
    print(f"\nwrote {OUT_DIR / 'results_betli.json'}")
