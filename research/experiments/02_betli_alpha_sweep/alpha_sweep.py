"""Sweep α from 0 to 2 (step 0.2), 50 seeds per α, fast betli solver.

Records per-α: sol win rate, mean / median / max solve time.

Usage:
    PYTHONPATH=. python3 experiments/01_betli_solver_speedup/alpha_sweep.py
"""
...
def solve_one(seed: int, alpha: float) -> tuple[str, float]:
    ...
    pos = pis_bridge.build_position(
        hands=hands, soloist=0, leader=0, contract="betli",
        talon=list(deal.talon),
    )
    t0 = time.perf_counter()
    vals = pis_bridge.solve_all(pos, contract="betli")
    dt = time.perf_counter() - t0
    best = max(vals.values())
    verdict = "soloist" if best >= _BETLI_WIN_VAL - 1e-6 else "defenders"
    return verdict, dt
