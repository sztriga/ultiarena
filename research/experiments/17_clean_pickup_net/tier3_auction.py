"""Tier 3 — calibrated, threshold-free auction using exp 17 multi-head net.

Copy of experiments/16_pre_pickup/auction_v2.py with one change:
loads Exp17Pickup from multihead_v17.pt instead of CalibratedPickupNet.
"""
...
def _load_cnet():
    global _CNET
    if _CNET is None:
        _CNET = Exp17Pickup.load(WEIGHTS)
    return _CNET


def _best_bid_above_rank(hand12, *, min_rank, ev_floor=None):
    cnet = _load_cnet()
...
def _oracle_evaluate(sol_hand_10, *, min_rank=0):
    cnet = _load_cnet()
...
def simulate(seed):
    ...
    if pick is None:
        ...
        return {
            'seed': seed, 'winner_pid': None, 'winning_bid': 'PASS_PENALTY',
            'n_pickups': 1, 'log': [{'pid': 0, 'action': 'open_pass_penalty'}],
    ...
    log.append({'pid': 0, 'action': 'open_bid',
        ...
        if next_pid == current['pid']:
            ...
            if cand is None:
                ...
                if cand['mean_ev'] <= pass_ev:
                    ...
                    pick = _best_bid_above_rank(new_hand12, min_rank=current['rank'])
                    if pick is None:
                        log.append({'pid': next_pid, 'action': 'pass',
                        ...
                        current = {'pid': next_pid, 'contract': cname2,
                                   ...
                                   'p_vnet': p_v2, 'ev': ev}
                        log.append({'pid': next_pid, 'action': 'overtake',
